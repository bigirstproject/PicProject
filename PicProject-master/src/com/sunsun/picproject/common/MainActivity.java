package com.sunsun.picproject.common;
...
public class MainActivity extends AnimationBaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addFragment(R.id.content_frame, new ContentListFragment(), XINGAN_FRAGMENT);
		...
		if (event != null) {
			if (event.getTable().getId() == 1) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), XINGAN_FRAGMENT);
			} else if (event.getTable().getId() == 2) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), SIWA_FRAGMENT);
			} else if (event.getTable().getId() == 3) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), GAOXIN_FRAGMENT);
			} else if (event.getTable().getId() == 4) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), WANGLUO_FRAGMENT);
			} else if (event.getTable().getId() == 5) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), WEIMEI_FRAGMENT);
			} else if (event.getTable().getId() == 6) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), TIYU_FRAGMENT);
			} else if (event.getTable().getId() == 7) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), MOTE_FRAGMENT);
			} else if (event.getTable().getId() == 8) {
				replaceFragment(R.id.content_frame,
						new MenuFragment(), MINGXING_FRAGMENT);
			}
			toggle();
		}
	}
